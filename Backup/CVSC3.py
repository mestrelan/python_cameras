...
def pos_execucao(file_name,nome_do_video,source_folder,pasta_pra_rodar,filtro="0",reset_dataset=False):
    ...
    if reset_dataset==True:
        path_nova_pasta_modelo=os.path.join(source_folder,"CVSC CNT2 "+str(filtro))
        ...
        if os.path.exists(os.path.join(Resultados_dos_CVSC,"CVSC CNT2 "+str(filtro))):
            #Se ela ja existe la, pode apagar
            shutil.rmtree(path_nova_pasta_modelo)
        if not os.path.exists(os.path.join(Resultados_dos_CVSC,"CVSC CNT2 "+str(filtro))):
