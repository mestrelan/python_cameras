...
def main(contador_frame_real,contador_frame,VIDEO_SOURCE,video_path_temp,tipo="RGB",filtro="0",video_out=False):
    #FMT
...
def criando_novo_teste(tipo="RGB",filtro="0"):
    pasta_pra_rodar = "dataset/Thermal/frame/Fall"
    
    if tipo == "RGB":
        source_folder = "Fall/RGB"
    if tipo == "IR":
        source_folder = "Fall"
...
def cvsc(animation_out=False):
    ####Timer
    inicio=time.time()
    #user input
    detection_threshold=0.3
    folder_path='dataset/Thermal/frame/Fall/Fall37'
    ...
    thermal_3dcae_path='mrfd/Thermal_track/ROI_Fusion/ROI_C3DAE-no_pool-BN_diff_ROI_C3DAE_no_pool-BN_Fusion_C3D-no_pool-BN/lambda_TS1.0_TT1.0_F1.0/models/GAN_T_R_weights_epoch-299.h5'
...
def pos_execucao(file_name,nome_do_video,source_folder,pasta_pra_rodar,filtro="0",reset_dataset=False):
    ...
    if reset_dataset==True:
        path_nova_pasta_modelo=os.path.join(source_folder,"CVSC MOG2 "+str(filtro))
        ...
        if os.path.exists(os.path.join(Resultados_dos_CVSC,"CVSC MOG2 "+str(filtro))):
            #Se ela ja existe la, pode apagar
            shutil.rmtree(path_nova_pasta_modelo)
        if not os.path.exists(os.path.join(Resultados_dos_CVSC,"CVSC MOG2 "+str(filtro))):
